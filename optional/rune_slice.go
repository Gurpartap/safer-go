// Code generated by optionalize
// on: 2017-10-19 10:36:00.357127913 &#43;0000 UTC
// from: github.com/Gurpartap/safer-go/optional/template/optional.go.tmpl

package optional

import (
	"bytes"
	"encoding/json"
	"fmt"

	"github.com/pkg/errors"
)

type RuneSlice interface {
	// Take takes the value out of the option, leaving a None in its place.
	Take()

	// From performs the conversion.
	From(value []rune)

	// Unwrap moves the value out of the optional, if it is Some(value).
	// This function returns multiple values, and if that's undesirable,
	// consider using Some and None functions.
	Unwrap() ([]rune, bool)

	// UnwrapOr returns the contained value or a default.
	UnwrapOr(def []rune) []rune

	// UnwrapOrElse returns the contained value or computes it from a closure.
	UnwrapOrElse(fn func() []rune) []rune

	// UnwrapOrPanic returns the contained value or panics.
	UnwrapOrPanic() []rune

	// And returns None if the optional is None, otherwise returns optb.
	And(optb RuneSlice) RuneSlice

	// Or returns the optional if it contains a value, otherwise returns optb.
	Or(optb RuneSlice) RuneSlice

	// OrElse returns the optional if it contains a value, otherwise calls fn
	// and returns the result.
	OrElse(fn func() RuneSlice) RuneSlice

	// IsSome returns true if the optional is a Some value.
	IsSome() bool

	// IsNone returns true if the optional is a None value.
	IsNone() bool

	// Some executes the given closure if there is a Some value.
	Some(fn func(value []rune)) someRuneSliceHandler

	// None executes the given closure if there is a None value.
	None(fn func())
}

type someRuneSliceHandler interface {
	None(fn func())
}

type _RuneSlice struct {
	hasValue    bool
	unsafeValue []rune
}

func NewRuneSlice() RuneSlice {
	opt := &_RuneSlice{}
	opt.Take()
	return opt
}

func RuneSliceFrom(value []rune, hasValue bool) RuneSlice {
	opt := &_RuneSlice{}
	if hasValue {
		opt.From(value)
	}
	return opt
}

// Take takes the value out of the optional, leaving a None in its place.
func (opt *_RuneSlice) Take() {
	var value []rune
	opt.unsafeValue = value
	opt.hasValue = false
}

// From performs the conversion.
func (opt *_RuneSlice) From(value []rune) {
	opt.unsafeValue = value
	opt.hasValue = true
}

// Unwrap moves the value out of the optional, if it is Some(value).
// This function returns multiple values, and if that's undesirable,
// consider using Some and None functions.
func (opt *_RuneSlice) Unwrap() ([]rune, bool) {
	return opt.getUnsafeValue(), opt.getHasValue()
}

// UnwrapOr returns the contained value or a default.
func (opt *_RuneSlice) UnwrapOr(def []rune) []rune {
	if opt.getHasValue() {
		return opt.getUnsafeValue()
	}
	return def
}

// UnwrapOrElse returns the contained value or computes it from a closure.
func (opt *_RuneSlice) UnwrapOrElse(fn func() []rune) []rune {
	if opt.getHasValue() {
		return opt.getUnsafeValue()
	}
	return fn()
}

// UnwrapOrPanic returns the contained value or panics.
func (opt *_RuneSlice) UnwrapOrPanic() []rune {
	if opt.getHasValue() {
		return opt.getUnsafeValue()
	}
	panic("unable to unwrap RuneSlice")
}

// And returns None if the optional is None, otherwise returns optb.
func (opt *_RuneSlice) And(optb RuneSlice) RuneSlice {
	if !opt.getHasValue() {
		return opt
	}
	return optb
}

// Or returns the optional if it contains a value, otherwise returns optb.
func (opt *_RuneSlice) Or(optb RuneSlice) RuneSlice {
	if opt.getHasValue() {
		return opt
	}
	return optb
}

// OrElse returns the optional if it contains a value, otherwise calls fn and
// returns the result.
func (opt *_RuneSlice) OrElse(fn func() RuneSlice) RuneSlice {
	if opt.getHasValue() {
		return opt
	}
	return fn()
}

// IsSome returns true if the optional is a Some value.
func (opt *_RuneSlice) IsSome() bool {
	return opt.getHasValue()
}

// IsNone returns true if the optional is a None value.
func (opt *_RuneSlice) IsNone() bool {
	return !opt.getHasValue()
}

// Some executes the given closure if there is a Some value.
func (opt *_RuneSlice) Some(fn func(value []rune)) someRuneSliceHandler {
	if opt.getHasValue() {
		fn(opt.getUnsafeValue())
	}
	return _someRuneSliceHandler{opt: opt}
}

// None executes the given closure if there is a None value.
func (opt *_RuneSlice) None(fn func()) {
	if !opt.getHasValue() {
		fn()
	}
}

type _someRuneSliceHandler struct {
	opt *_RuneSlice
}

func (some _someRuneSliceHandler) None(fn func()) {
	if !some.opt.getHasValue() {
		fn()
	}
}

func (opt *_RuneSlice) getHasValue() bool {
	return opt.hasValue
}

func (opt *_RuneSlice) getUnsafeValue() []rune {
	return opt.unsafeValue
}

// String conforms to fmt Stringer interface.
func (opt *_RuneSlice) String() string {
	if value, ok := opt.Unwrap(); ok {
		return fmt.Sprintf("Some(%v)", value)
	}
	return "None"
}

// MarshalJSON implements the json Marshaler interface.
func (opt _RuneSlice) MarshalJSON() ([]byte, error) {
	if !opt.getHasValue() {
		return []byte("null"), nil
	}
	return json.Marshal(opt.getUnsafeValue())
}

// UnmarshalJSON implements the json Unmarshaler interface.
func (opt *_RuneSlice) UnmarshalJSON(data []byte) error {
	if bytes.Equal(data, []byte("null")) || data == nil {
		opt.Take()
		return nil
	}

	err := json.Unmarshal(data, &opt.unsafeValue)
	if err != nil {
		opt.hasValue = false
		return errors.WithStack(err)
	}
	opt.hasValue = true

	return nil
}
